                        ...
                        <div class="relative z-0 w-full mb-6 group">
                            ...
                            <div class="relative z-0 w-full mb-1 group">
                                ...
                                dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                                          ...
                                          style="height: 220px; resize:none;">{{ $project->description }}</textarea>
                ...
                <div class="w-2/4 px-2">
                    ...
                    <div class="inline-flex items-center">
                        <div class="grid grid-cols-4 gap-4">
                            @php
                                $projectAcademyIds = $project->profiles->pluck('id')->toArray();
                            @endphp

                            @foreach ($academies as $academy)
                                <input type="checkbox" class="form-check-input" name="academy[]"
                                       value="{{ $academy->id }}"
                                       id="academy{{ $academy->id }}" {{ (in_array($academy->id, $projectAcademyIds)) ? 'checked' : '' }}>
                                <label for="academy{{ $academy->id }}" id="academy{{ $academy->id }}"
                                       class="form-check-box card-square rounded-lg font-semibold shadow-sm small border rounded-2 m-1 {{ (in_array($academy->id, $projectAcademyIds)) ? 'bg-green text-white' : 'bg-light' }}">
                                    {{ $academy->name }}
                                </label>
                            @endforeach
                        </div>
